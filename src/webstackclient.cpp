...
void _ParseClientInfoPort(const char* port, size_t length, WebstackClientInfo& clientInfo)
{
...
{
    mujinjsonwebstack::LoadJsonValueByKey(rClientInfo, "host", host);
    mujinjsonwebstack::LoadJsonValueByKey(rClientInfo, "httpPort", httpPort);
    mujinjsonwebstack::LoadJsonValueByKey(rClientInfo, "username", username);
    mujinjsonwebstack::LoadJsonValueByKey(rClientInfo, "password", password);
    mujinjsonwebstack::LoadJsonValueByKey(rClientInfo, "additionalHeaders", additionalHeaders);
    mujinjsonwebstack::LoadJsonValueByKey(rClientInfo, "unixEndpoint", unixEndpoint);
}
...
void WebstackClientInfo::SaveToJson(rapidjson::Value& rClientInfo, rapidjson::Document::AllocatorType& alloc) const
{
    ...
    if( !host.empty() ) {
        mujinjsonwebstack::SetJsonValueByKey(rClientInfo, "host", host, alloc);
    }
    if( httpPort != 0 ) {
        mujinjsonwebstack::SetJsonValueByKey(rClientInfo, "httpPort", httpPort, alloc);
    }
    if( !username.empty() ) {
        mujinjsonwebstack::SetJsonValueByKey(rClientInfo, "username", username, alloc);
    }
    if( !password.empty() ) {
        mujinjsonwebstack::SetJsonValueByKey(rClientInfo, "password", password, alloc);
    }
    if( !additionalHeaders.empty() ) {
        mujinjsonwebstack::SetJsonValueByKey(rClientInfo, "additionalHeaders", additionalHeaders, alloc);
    }
    if( !unixEndpoint.empty() ) {
        mujinjsonwebstack::SetJsonValueByKey(rClientInfo, "unixEndpoint", unixEndpoint, alloc);
    }
}
